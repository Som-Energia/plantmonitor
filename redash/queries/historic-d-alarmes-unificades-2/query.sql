select * from
(
    select plant.id as plant_id, plant.name as plant_name, device_id, device_name as device_name, alarm.name as alarm_name, start_time, end_time
    from alarm_historic
...
WHERE 0 IN ({{ plant }}) OR plant_id IN ({{ plant }})
  AND end_time between '{{ interval.start }}' and '{{ interval.end }}';