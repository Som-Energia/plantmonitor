select time at time zone 'Europe/Madrid' as temps,
    ...
    case when now() at time zone 'Europe/Madrid' - interval '9 minutes' < time then (
...
from 
    ...
    where (sub.sonda_zero or sub.sonda_12_equal or sub.sonda_massa_irradiacio or sub.time > now() at time zone 'Europe/Madrid' - interval '9 minutes')
    --TODO union instead of case now to support status outside sunlit hours
        order by temps desc;