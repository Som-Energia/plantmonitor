 SELECT date_trunc('month'::text, result.day_date) AS month_date,
  ...
  GROUP BY (date_trunc('month'::text, result.day_date)), result.inverter_name
  ORDER BY (date_trunc('month'::text, result.day_date));
