 SELECT timezone('Europe/Madrid'::text, sistema_contador."time") AS timezone,
   ...
   FROM (
     ...
     JOIN public.sistema_contador
	ON integrated_sensors."time" = timezone('UTC'::text, sistema_contador."time")),
    LATERAL (
	VALUES (
		((sistema_contador.export_energy)::double precision / (2160.0)::double precision) / (NULLIF((integrated_sensors.integral_irradiation_wh_m2)::double precision, (0.0)::double precision) / (1000000.0)::double precision)
		)
	) raw_pr_tmp(pr_hourly_raw);
